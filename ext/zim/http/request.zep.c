...
PHP_METHOD(Zim_Http_Request, initialize) {
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_5, "__construct", NULL, 91, &_7);
	zephir_check_call_status();
...
PHP_METHOD(Zim_Http_Request, createFromGlobals) {
	...
	ZEPHIR_INIT_VAR(&_0);
	array_init(&_0);
	ZEPHIR_CALL_METHOD(NULL, &request, "__construct", NULL, 92, _GET, _POST, &_0, _SERVER);
	zephir_check_call_status();
	...
	if (_5) {
		ZEPHIR_CALL_METHOD(&_11$$3, &request, "getcontent", NULL, 93);
		zephir_check_call_status();
		ZEPHIR_MAKE_REF(&data);
		ZEPHIR_CALL_FUNCTION(NULL, "parse_str", NULL, 94, &_11$$3, &data);
		ZEPHIR_UNREF(&data);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_12$$3);
		object_init_ex(&_12$$3, zim_http_parameterbag_ce);
		ZEPHIR_CALL_METHOD(NULL, &_12$$3, "__construct", NULL, 90, &data);
		zephir_check_call_status();
...
PHP_METHOD(Zim_Http_Request, __toString) {
	...
	if (EG(exception)) {
		...
		if (zephir_instance_of_ev(&_0, spl_ce_LogicException TSRMLS_CC)) {
			...
			ZEPHIR_CPY_WRT(&e, &_0);
			ZVAL_LONG(&_1$$4, 256);
			ZEPHIR_RETURN_CALL_FUNCTION("trigger_error", NULL, 95, &e, &_1$$4);
			zephir_check_call_status();
			RETURN_MM();
		}
	}
...
PHP_METHOD(Zim_Http_Request, normalizeQueryString) {
	...
	ZEPHIR_MAKE_REF(&qs);
	ZEPHIR_CALL_FUNCTION(NULL, "parse_str", NULL, 94, &qs, &qs);
	ZEPHIR_UNREF(&qs);
	zephir_check_call_status();
	ZEPHIR_MAKE_REF(&qs);
	...
	ZVAL_STRING(&_1, "&");
	ZVAL_LONG(&_2, 2);
	ZEPHIR_RETURN_CALL_FUNCTION("http_build_query", NULL, 96, &qs, &_0, &_1, &_2);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Zim_Http_Request, getContent) {
	...
	if (asResource == 1) {
		if (zephir_is_true(&currentContentIsResource)) {
			...
			ZEPHIR_CALL_FUNCTION(NULL, "rewind", &_2, 97, &_1$$4);
			zephir_check_call_status();
		...
		ZVAL_STRING(&_9$$3, "rb");
		ZEPHIR_RETURN_CALL_FUNCTION("fopen", &_6, 98, &_8$$3, &_9$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	if (zephir_is_true(&currentContentIsResource)) {
		zephir_read_property(&_10$$6, this_ptr, SL("content"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_FUNCTION(NULL, "rewind", &_2, 97, &_10$$6);
		zephir_check_call_status();
		zephir_read_property(&_11$$6, this_ptr, SL("content"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_RETURN_CALL_FUNCTION("stream_get_contents", NULL, 99, &_11$$6);
		zephir_check_call_status();
		RETURN_MM();
	}
