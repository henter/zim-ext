...
PHP_METHOD(Zim_Support_Str, replaceFirst) {
	...
	if (!ZEPHIR_IS_FALSE_IDENTICAL(&position)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("substr_replace", NULL, 67, &subject, &replace, &position, &_0$$4);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_CTOR(&subject);
...
PHP_METHOD(Zim_Support_Str, replaceLast) {
	...
	ZEPHIR_CALL_FUNCTION(&position, "strrpos", NULL, 64, &subject, &search);
	zephir_check_call_status();
	if (!ZEPHIR_IS_FALSE_IDENTICAL(&position)) {
		ZVAL_LONG(&_0$$3, zephir_fast_strlen_ev(&search));
		ZEPHIR_RETURN_CALL_FUNCTION("substr_replace", NULL, 67, &subject, &replace, &position, &_0$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_CTOR(&subject);
